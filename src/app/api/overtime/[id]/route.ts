...
// Server-side PATCH/DELETE for a single overtime request (مدير/إداري only).
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, OVERTIME_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'طلب العمل الإضافي غير موجود' }, { status: 404 });
    console.error('خطأ في تحديث طلب العمل الإضافي:', err);
    return NextResponse.json({ error: 'فشل تحديث طلب العمل الإضافي' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, OVERTIME_COLLECTION_ID, id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'طلب العمل الإضافي غير موجود' }, { status: 404 });
    console.error('خطأ في حذف طلب العمل الإضافي:', err);
    return NextResponse.json({ error: 'فشل حذف طلب العمل الإضافي' }, { status: 500 });
  }
}