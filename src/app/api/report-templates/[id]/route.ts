...
// Server-side PATCH/DELETE for a single report template (مدير/إداري only).
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, REPORT_TEMPLATES_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'القالب غير موجود' }, { status: 404 });
    console.error('خطأ في تحديث القالب:', err);
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, REPORT_TEMPLATES_COLLECTION_ID, id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'القالب غير موجود' }, { status: 404 });
    console.error('خطأ في حذف القالب:', err);
