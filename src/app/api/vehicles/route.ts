...
// Server-side CRUD for vehicles (مدير/إداري only, node-appwrite + API key).
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, VEHICLES_COLLECTION_ID, [
      ...queries.map((q) => {
    ...
    console.error('خطأ في جلب المركبات:', err);
    return NextResponse.json({ error: 'فشل جلب المركبات' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(auth.session.email, 'vehicles-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم المركبة مفقود' }, { status: 400 });
  }
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, VEHICLES_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم المركبة موجود بالفعل' }, { status: 409 });
    console.error('خطأ في إنشاء المركبة:', err);
    return NextResponse.json({ error: 'فشل إنشاء المركبة' }, { status: 500 });
  }
}