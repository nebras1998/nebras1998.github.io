...
// Server-side PATCH/DELETE for a single vehicle (مدير/إداري only).
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, VEHICLES_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'المركبة غير موجودة' }, { status: 404 });
    console.error('خطأ في تحديث المركبة:', err);
    return NextResponse.json({ error: 'فشل تحديث المركبة' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, VEHICLES_COLLECTION_ID, id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'المركبة غير موجودة' }, { status: 404 });
    console.error('خطأ في حذف المركبة:', err);
    return NextResponse.json({ error: 'فشل حذف المركبة' }, { status: 500 });
  }
}