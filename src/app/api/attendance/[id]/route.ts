...
// Server-side PATCH/DELETE for a single attendance record (مدير/إداري/فني only).
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, ATTENDANCE_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'سجل الحضور غير موجود' }, { status: 404 });
    console.error('خطأ في تحديث سجل الحضور:', err);
    return NextResponse.json({ error: 'فشل تحديث سجل الحضور' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, ATTENDANCE_COLLECTION_ID, id);
