...
// Notifications PATCH is restricted to the 'read' field only (mark-as-read).
// Server-side PATCH for a single notification (مدير/إداري/فني only).
...
const NOTIFICATION_EDITABLE_FIELDS = ['isRead'];
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  for (const [key, value] of Object.entries(body)) {
    if (!key.startsWith('$') && NOTIFICATION_EDITABLE_FIELDS.includes(key)) cleanData[key] = value;
  }
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, NOTIFICATIONS_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'التنبيه غير موجود' }, { status: 404 });
    console.error('خطأ في تحديث التنبيه:', err);
