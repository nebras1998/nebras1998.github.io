...
// Server-side GET/POST for the vehicle trips collection (مدير/إداري/فني only).
// The vehicle trips collection is locked down to read("users") only.
...
// GET /api/vehicle-trips — list vehicle trips (مدير/إداري/فني)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, VEHICLE_TRIPS_COLLECTION_ID, [
      ...queries.map((q) => {
    ...
    console.error('خطأ في جلب رحلات المركبات:', err);
    return NextResponse.json({ error: 'فشل جلب رحلات المركبات' }, { status: 500 });
  }
}

// POST /api/vehicle-trips — create a vehicle trip (مدير/إداري/فني)
export async function POST(request: NextRequest) {
  const auth = await authenticate(request);
  if (!auth.ok) return auth.response;

  const rateKey = sessionRateLimitKey(auth.session.email, 'vehicle-trips-create');
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, VEHICLE_TRIPS_COLLECTION_ID, ID.unique(), cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم الرحلة موجود بالفعل' }, { status: 409 });
    console.error('خطأ في إنشاء الرحلة:', err);
    return NextResponse.json({ error: 'فشل إنشاء الرحلة' }, { status: 500 });
  }
}