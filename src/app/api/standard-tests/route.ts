// src/app/api/standard-tests/route.ts
// Server-side CRUD for standard tests (مدير/إداري only, node-appwrite + API key).

import { NextRequest, NextResponse } from 'next/server';
import { Client, Databases, Query } from 'node-appwrite';
import { DATABASE_ID, STANDARD_TESTS_COLLECTION_ID } from '@/lib/constants';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await databases.listDocuments(DATABASE_ID, STANDARD_TESTS_COLLECTION_ID, queries);
    return NextResponse.json(res);
  } catch (err) {
    console.error('خطأ في جلب الفحوصات القياسية:', err);
    return NextResponse.json({ error: 'فشل جلب الفحوصات القياسية' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(account.email, 'standard-tests-create');
  ...
  try {
    const client = new Client()
      ...
      .setKey(apiKey);
    const databases = new Databases(client);
    const doc = await databases.createDocument(DATABASE_ID, STANDARD_TESTS_COLLECTION_ID, documentId, cleanData);
