...
// Server-side CRUD for equipment (مدير/إداري only, node-appwrite + API key).
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, EQUIPMENT_COLLECTION_ID, [
      ...queries.map((q) => {
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(auth.session.email, 'equipment-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم الجهاز مفقود' }, { status: 400 });
  }
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, EQUIPMENT_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم الجهاز موجود بالفعل' }, { status: 409 });
    console.error('خطأ في إنشاء الجهاز:', err);
    return NextResponse.json({ error: 'فشل إنشاء الجهاز' }, { status: 500 });
  }
}