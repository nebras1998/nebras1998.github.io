...
// Server-side PATCH/DELETE for a single project (مدير/إداري only).

import { NextRequest, NextResponse } from 'next/server';
import { Client, Databases, Query } from 'node-appwrite';
import { DATABASE_ID, PROJECTS_COLLECTION_ID, SAMPLES_COLLECTION_ID, TESTS_COLLECTION_ID } from '@/lib/constants';
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, PROJECTS_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'المشروع غير موجود' }, { status: 404 });
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [samples, tests] = await Promise.all([
      databases.listDocuments(DATABASE_ID, SAMPLES_COLLECTION_ID, [Query.equal('projectId', id), Query.limit(1)]),
      databases.listDocuments(DATABASE_ID, TESTS_COLLECTION_ID, [Query.equal('projectId', id), Query.limit(1)]),
    ]);
    if (samples.total > 0) {
      return NextResponse.json({ error: `لا يمكن حذف المشروع لأن لديه ${samples.total} عينة مرتبطة` }, { status: 409 });
    }
    if (tests.total > 0) {
      return NextResponse.json({ error: `لا يمكن حذف المشروع لأن لديه ${tests.total} فحص/فحوصات مرتبطة` }, { status: 409 });
    }
