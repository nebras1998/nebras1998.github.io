...
// Server-side CRUD for projects (مدير/إداري only, node-appwrite + API key).
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, PROJECTS_COLLECTION_ID, [
      ...queries.map((q) => {
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(auth.session.email, 'projects-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم المشروع مفقود' }, { status: 400 });
  }
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, PROJECTS_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم المشروع موجود بالفعل' }, { status: 409 });
