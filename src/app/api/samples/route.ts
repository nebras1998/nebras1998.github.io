...
// Server-side CRUD for samples (مدير/إداري only, node-appwrite + API key).

import { NextRequest, NextResponse } from 'next/server';
import { Client, Databases } from 'node-appwrite';
import { DATABASE_ID, SAMPLES_COLLECTION_ID } from '@/lib/constants';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, SAMPLES_COLLECTION_ID, [
      ...queries.map((q) => {
    ...
    console.error('خطأ في جلب العينات:', err);
    return NextResponse.json({ error: 'فشل جلب العينات' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(auth.session.email, 'samples-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم العينة مفقود' }, { status: 400 });
  }
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, SAMPLES_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم العينة موجود بالفعل' }, { status: 409 });
    console.error('خطأ في إنشاء العينة:', err);
    return NextResponse.json({ error: 'فشل إنشاء العينة' }, { status: 500 });
  }
}