// src/app/api/sample-types/route.ts
// Server-side CRUD for sample types (مدير/إداري only, node-appwrite + API key).

import { NextRequest, NextResponse } from 'next/server';
import { Client, Databases, Query } from 'node-appwrite';
import { DATABASE_ID, SAMPLE_TYPES_COLLECTION_ID } from '@/lib/constants';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await databases.listDocuments(DATABASE_ID, SAMPLE_TYPES_COLLECTION_ID, queries);
    return NextResponse.json(res);
  } catch (err) {
    console.error('خطأ في جلب أنواع العينات:', err);
    return NextResponse.json({ error: 'فشل جلب أنواع العينات' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(account.email, 'sample-types-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم النوع مفقود' }, { status: 400 });
  }
  ...
  try {
    const client = new Client()
      ...
      .setKey(apiKey);
    const databases = new Databases(client);
    const doc = await databases.createDocument(DATABASE_ID, SAMPLE_TYPES_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم النوع موجود بالفعل' }, { status: 409 });
    console.error('خطأ في إنشاء النوع:', err);
    return NextResponse.json({ error: 'فشل إنشاء النوع' }, { status: 500 });
  }
}
