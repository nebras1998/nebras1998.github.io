...
// Server-side CRUD for leave requests (مدير/إداري only, node-appwrite + API key).
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const page = await databases.listDocuments(DATABASE_ID, LEAVE_REQUESTS_COLLECTION_ID, [
      ...queries.map((q) => {
    ...
    return NextResponse.json({ error: 'فشل جلب طلبات الإجازة' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const rateKey = sessionRateLimitKey(auth.session.email, 'leaves-create');
  ...
  if (!documentId) {
    return NextResponse.json({ error: 'رقم طلب الإجازة مفقود' }, { status: 400 });
  }
  ...
  try {
    const { databases } = createApiKeyClient();
    const doc = await databases.createDocument(DATABASE_ID, LEAVE_REQUESTS_COLLECTION_ID, documentId, cleanData);
    return NextResponse.json(doc, { status: 201 });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 409) return NextResponse.json({ error: 'رقم طلب الإجازة موجود بالفعل' }, { status: 409 });
