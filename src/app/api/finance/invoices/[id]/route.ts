...
// Server-side PATCH/DELETE for a single invoice. Writes use node-appwrite +
// APPWRITE_API_KEY and are restricted to مدير/إداري roles.
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, INVOICES_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'الفاتورة غير موجودة' }, { status: 404 });
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, INVOICES_COLLECTION_ID, id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'الفاتورة غير موجودة' }, { status: 404 });
    console.error('خطأ في حذف الفاتورة:', err);
    return NextResponse.json({ error: 'فشل حذف الفاتورة' }, { status: 500 });
  }
}