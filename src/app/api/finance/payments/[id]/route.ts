...
// Server-side PATCH/DELETE for a single payment (مدير/إداري only).
...
export async function PATCH(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await databases.updateDocument(DATABASE_ID, PAYMENTS_COLLECTION_ID, id, cleanData);
    return NextResponse.json(updated);
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'الدفعة غير موجودة' }, { status: 404 });
    console.error('خطأ في تحديث الدفعة:', err);
    return NextResponse.json({ error: 'فشل تحديث الدفعة' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { databases } = createApiKeyClient();
    await databases.deleteDocument(DATABASE_ID, PAYMENTS_COLLECTION_ID, id);
    return NextResponse.json({ ok: true });
  } catch (err) {
    const code = (err as { code?: number } | null)?.code;
    if (code === 404) return NextResponse.json({ error: 'الدفعة غير موجودة' }, { status: 404 });
    console.error('خطأ في حذف الدفعة:', err);
    return NextResponse.json({ error: 'فشل حذف الدفعة' }, { status: 500 });
  }
}