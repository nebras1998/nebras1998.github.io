...
const TYPE_CODES: Record<string, string> = {
  'خرسانة': 'CON', 'تربة': 'SOI', 'أسفلت': 'ASP', 'ركام': 'AGG', 'مياه': 'WAT', 'فولاذ': 'STL', 'حجر': 'STN', 'بلاط': 'TIL', 'أخرى': 'OTH',
};
...
export default function EditSamplePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [sample, projectsRes, techRes, typesRes] = await Promise.all([
        ...
        // جلب الفحوصات المرتبطة بالنوع الحالي
        const typeDoc = typesRes.documents.find((t: any) => t.name === sample.type);
        if (typeDoc) {
  ...
  useEffect(() => {
    ...
    // عند تغيير النوع، نجلب فحوصاته
    const typeDoc = sampleTypes.find(t => t.name === formData.type);
    if (typeDoc) {
  ...
  }, [formData.type, formData.samplingDate, sampleTypes]);
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); setSaving(true);
    try {
      await databases.updateDocument(DATABASE_ID, SAMPLES_COLLECTION_ID, sampleId, formData);
      toast.success('تم تحديث العينة بنجاح');
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {standardTests.length > 0 && (
            <div className="bg-green-50 p-4 rounded-lg">
              <h3 className="font-bold mb-2 text-green-800">الفحوصات المتاحة (لن تؤثر على الفحوصات المنشأة)</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {standardTests.map((test) => (
                  <label key={test.$id} className="flex items-center gap-2 text-sm">
                    <input type="checkbox" checked={selectedTests.includes(test.$id)} onChange={() => toggleTestSelection(test.$id)} />
